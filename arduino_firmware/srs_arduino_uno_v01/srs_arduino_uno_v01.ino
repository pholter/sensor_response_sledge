...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    if (inChar == 'V') {
        Serial.print(">>> --- --- <<<\n");
        Serial.print(">>> --- --- <<<\n");
        Serial.print(">>> --- SRS firmware v1.0 (Peter Holtermann peter.holtermann@io-warnemuende.de) ---\n");
  ...
  if(t50Hzcount >= 10) // 50 Hz
    {
      ...
      if(  ( (counter - counter_lastena) > 500 ) && ( FLAG_AUTO_STOP == 1 ) )
      {
        ena = 1;
      }
      if(ena == 1)
